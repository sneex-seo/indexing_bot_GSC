...
def indexing_method(message):
    ...
    for link_for_index in indexing_list:
        ...
        if("error" in result):
            ...
            print("urlNotificationMetadata.latestRemove.notifyTime: {}".format(result["urlNotificationMetadata"]["latestRemove"]["notifyTime"]))
            bot.send_message(message.chat.id, 'Удалил ссылку: ' + link_for_index)
